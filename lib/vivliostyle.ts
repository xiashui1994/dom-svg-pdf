...
import { allStylesheets, patchStyles } from './styles'
...
export async function domPaged(el: HTMLElement, options?: PDFOptions): Promise<Paged & { iframeWin: Window }> {
  ...
  await beforePaged?.()
  const htmlDoc = `<!doctype html><html><head>${allStylesheets()}<style>${patchStyles(docDefinition.defaultStyle?.font, katex)}</style></head><body>${el.innerHTML}</body></html>`
